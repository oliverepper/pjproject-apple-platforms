...
function createLib {
    ...
    EXTRA_LIBS=""
    if [ -d "${OPUS[@]: -1}" ]; then
        EXTRA_LIBS=""${OPUS[@] -1}"/lib/libopus.a"
    fi
    if [[ -d "${SDL[@]: -1}" ]]; then
        EXTRA_LIBS+=" "${SDL[@]: -1}"/lib/libSDL2.a"
    fi
    echo "Running: libtool -static -o libpjproject.a ./*.a "${EXTRA_LIBS}""
...
if [[ -d "${SDL[@]: -1}" ]]; then
    CONFIGURE_EXTRA_PARAMS+=" --with-sdl="${SDL[@]: -1}""
fi
